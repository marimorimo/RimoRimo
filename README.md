...
<aside>
🥊 마이 페이지
    |최종 마리모를 사용자가 선택한 마리모로 보여줘야 하는데 이미지(URL)로 변환하여 저장한 데이터를 변수에 다시 저장하여 보여주려고 하니 제대로 이미지가 보이지 않는 상황이 발생|처음부터 assets에 있는 이미지 이름을 저장하고, 문자열로서 변수에 저장하여 보여주는 방식으로 처리했습니다.| 
    |타이머의 시간과 마리모의 시간을 별도로 두고 서로 연동 및 백그라운드에서도 동작하게 구현 |상태를 UserDefaults에 저장하는 방식으로 해결| 
